...
class Provider extends events.EventEmitter {
   ...
   * @param req
   */
  generateGoogleTagManager(req) {
      const results = [];
      const GL = '_gl';
      if (!req.cookies) return results;
      if (!req.cookies[GL]);
      return results.push([GL, req.cookies[GL]])
  }
  ...
  async interactionFinished(req, res, result, { mergeWithLastSubmission = true } = {}) {
    ...
    const gtmParams = this.generateGoogleTagManager(req);
    const query = this.generateQueryParams(gtmParams);

    res.statusCode = 303; // eslint-disable-line no-param-reassign
    res.setHeader('Location', `${returnTo}${query}`);
